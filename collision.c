...
void Act(void) { 
    ...
    /*공과 선수가 부H혔을때*/
    int i=0;
    for( i=0;i<BUFSIZE2;i++){ //player가 아니구  Character[i].player[j]형태로 받아져야함
    if(CheckCollisionObjects(&BALL,&(player[i]))){
        elasticCollision2D(&BALL,&(player[i]));   
    }
    }
    /*공과 경기장이 부H혔을때*/
    if((BALL.pos.x==0 && (BALL.pos.y>0 && BALL.pos.y<410)) ||((BALL.pos.x>0 && BALL.pos.x<640 ) && BALL.pos.y==0) ||(BALL.pos.x==640 && (BALL.pos.y>0 && BALL.pos.y<410))){
